...
radiation._readEntity = function ( entityDef ) {
	return new Promise( async (resolve, reject) => {
		try {
...
radiation.listen = function (listener) {
	...
	return new Promise( (resolve, reject) => {
		if ( !listener || !_.isObject( listener ) )
			return reject( new Error( 'Proper listener object must be passed.' ) )
...
radiation.revoke = function ( firestarter, protocol, revokerFN ) {
	...
	return new Promise( async (resolve, reject) => {
		if ( !self[protocol] )
			return reject( new Error( protocol + ' has not been started yet.', firestarter.name ) )
