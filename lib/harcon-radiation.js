...
radiation.checkSecurity = function( data ){
	for(var fName in this.ios) {
		if( fName && this.ios[fName] ){
			var fs = this.ios[fName].firestarter;
			if( fs.matches( data.division, data.event ) )
				return !fs.object.security || !fs.object.security.apiKeys || fs.object.security.apiKeys.contains( data.apiKey );
		}
	}
	return false;
};
...
radiation.io = function( io ) {
	...
	self.ioNamespace.on('connection', function(socket){
		socket.on('ignite', function( data ){
			if( data && data.event && self.checkSecurity(data) ){
