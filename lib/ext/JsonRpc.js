...
module.exports = {
	...
	processJsonRpc: async function ( request, content ) {
		...
		try {
			...
			let res = await self.request( content.id, { clientRequest: newRequest, content: content }, content.division || self.inflicter.division, content.method, params )
	...
	processJSONSocketData: async function ( socket, data ) {
		...
		try {
			...
			let res = await self.request( data.id, { content: data }, data.division || self.inflicter.division, data.method, params)
