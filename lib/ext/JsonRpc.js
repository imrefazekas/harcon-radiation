...
module.exports = {
	...
	processJSONSocketData: function( socket, data ){
		...
		if( self.checkJSONData( data ) )
			return self.sendJSONSocketError( data.id, JSON_RPC_ERRORS.parseError, '', callback );

		var params = data.params || [];
		self.ignite( data.id, { content: data }, '', data.method, params, function(err, res){
			if( err ) return self.sendJSONSocketError( data.id, JSON_RPC_ERRORS.internalError, err.message, callback );

			var data = res && res.length>0 ? res[0] : null;
			self.sendJSONSocketAnswer( content.id, data, socket );
		} );
	}
};
