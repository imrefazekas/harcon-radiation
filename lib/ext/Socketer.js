...
module.exports = {
	...
	checkSecurity: function ( data ) {
		...
		return new Promise( async (resolve, reject) => {
			console.log('--!!!!----', self.options.passthrough )
			...
			for (let fName in self.Websocket) {
				if ( fName && self.Websocket[fName] ) {
					console.log('---++++---', fName, data.event )
					let fs = self.Websocket[fName].firestarter
					console.log('!!!!+++---', fs.name, fs.matches( comm ) )
	...
	broadcast: function ( shiftReport ) {
		...
		return new Promise( async (resolve, reject) => {
			if (!_.isObject( shiftReport.state) )
				return reject( new Error('Object\'s state is not an object...'), shiftReport.state )
	...
	desocket: async function ( name, firestarter ) {
		var self = this
		return new Promise( async (resolve, reject) => {
			await self.revoke( firestarter, 'Websocket', function (firestarter, websocket) {
