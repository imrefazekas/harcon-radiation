...
var Communication = require( '../../node_modules/harcon/lib/Communication' );
...
module.exports = {
	checkSecurity: function( data, callback ){
		...
		var comm = Communication.newCommunication(null, null, data.id, self.firestarter.name, data.division, data.event, data.params );
		for(var fName in self.Websocket) {
			if( fName && self.Websocket[fName] ){
				var fs = self.Websocket[fName].firestarter;
