...
module.exports = {
	...
	broadcast: async function ( shiftReport ) {
		...
		for ( let key of Object.keys( shiftReport.state ) )
			self.fastify.ws.broadcast( JSON.stringify( {
				division: 'King',
				name: 'Marie',
				state: key,
	...
	ws: async function ( fastify ) {
		...
		fastify.register( require('../../util/WsPlugin').addWSServer(
			self.options.websocket.harconPath, async function ( socket, data ) {
				try {
					if ( _.isString(data) )
