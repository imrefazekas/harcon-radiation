	...
	rester: async function ( fastify ) {
		...
		if ( !self.options.rest.ignoreRESTPattern ) {
			self.fastify.post( '/:division/:entity/:event', async function (request, reply) {
				...
				let res = await self.ignite( clerobee.generate(), { request: newRequest, content: content }, division, event, Array.isArray( content ) ? content : content.params )
		...
		if ( self.options.rest.jsonrpcPath ) {
			...
			self.fastify.post( self.options.rest.jsonrpcPath, async function ( request, reply ) {
				...
				// await self.notifyListeners( 'posted', [ self, { request: newRequest, content: content } ] )
				let id = content.id
				let res = await self.ignite( id, { request: newRequest, content: content }, content.division, content.method, content.params )
